#setwd("D:/RdataRepo/WorkindDir/Capstone")
# Librerias and the not in function
library(shiny)
library(stringi)    
library(dplyr)      
library(quanteda)   # For N-Grams
`%notin%` <- function(x,y) !(x %in% y)   # <-- look at this... Tried something different.


# Load the n-gram data
ngram1 <- readRDS("D:/RdataRepo/WorkindDir/Capstone/ngram1.rds")
ngram2 <- readRDS("D:/RdataRepo/WorkindDir/Capstone/ngram2.rds")
ngram3 <- readRDS("D:/RdataRepo/WorkindDir/Capstone/ngram3.rds")
ngram4 <- readRDS("D:/RdataRepo/WorkindDir/Capstone/ngram4.rds")
ngram5 <- readRDS("D:/RdataRepo/WorkindDir/Capstone/ngram5.rds")
...
textpred <- function(text, nsug=10)  {
  ...
  # A fix for NULL
  if (nchar(text)==0) {
    return("Don't be shy (='.'=) Type a word  --- Input the text---")
  } else {
...
shinyServer(
  function(input, output) {
    ...
    output$nextword <- renderPrint({
      word <- textpred(input$text, input$nsug)
